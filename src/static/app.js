document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
      ...
      // Clear and reset activity select dropdown (preserve current selection if any)
        ...
        if (details.participants.length > 0) {
          const participantsHTML = details.participants.map(email => 
            `<li>
              ...
              <span class="delete-icon" onclick="unregisterParticipant('${name}', '${email}')" title="Remove participant">✕</span>
            </li>`
          ).join('');
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Store the current activity before resetting the form
        ...
        // Restore the selected activity in the dropdown
